<Cabbage>
form caption("Birds"), size(400, 950), colour(25, 25, 25), pluginId("bird")
label bounds(10, 10, 380, 20), text("Birds"), fontColour(255, 255, 255), font("default", 16)
...
hslider bounds(10, 125, 380, 30), channel("Octave"), range(0, 6, 3, 1, 1), text("Octave"), trackerColour("blue")
...
hslider bounds(10, 300, 380, 30), channel("ModIntensity"), range(0, 1, 0.25, 1, 0.01), text("Mod Intensity"), trackerColour("yellow")
...
hslider bounds(10, 405, 380, 30), channel("Squawk"), range(0, 1, 0, 1, 0.01), text("Squawk"), trackerColour("orange")
...
kTriggerID chnget "TriggerID"  ; Unique ID for each trigger
...
; Apply octave shift and read global frequency (gkFreq)
kOctaveMultiplier = pow(2, kOctave - 1) * 2
kGlobalFreq chnget "gkFreq"
kAdjustedFreq = kGlobalFreq * kOctaveMultiplier

iModIntensity = iModIntensity * 50 + 0.1

if changed(kTriggerID) == 1 then
    ; Randomize modulation frequency and bandwidth once per trigger
    giModFreq = random(1, 16)  ; Recalculate only if there's a new trigger
...
; Adjust modulation frequency relative to Squawk amount
kModFreqAdjusted = (1 - kSquawk) * kRandomModFreq + kSquawk * 0.1
...
aModSine poscil iModIntensity, kModFreqAdjusted, giSine
aModSaw poscil iModIntensity * gkSawNormFactor, kModFreqAdjusted, giSaw
aModSquare poscil iModIntensity * gkSquareNormFactor, kModFreqAdjusted, giSquare
aModTriangle poscil iModIntensity, kModFreqAdjusted, giTriangle
...
; Crow squawk formant frequencies
kFormantFreq1 = 600
kFormantFreq2 = 1000
kFormantFreq3 = 1600
kFormantFreq4 = 2400
...
; Resonance bandwidth for each formant
kBandwidth1 = 100
kBandwidth2 = 120
kBandwidth3 = 150
kBandwidth4 = 180
...
; Generate resonances for each formant
aRes1 = reson(aFilteredCarrier, kFormantFreq1, kBandwidth1) * 0.08
aRes2 = reson(aFilteredCarrier, kFormantFreq2, kBandwidth2) * 0.12
aRes3 = reson(aFilteredCarrier, kFormantFreq3, kBandwidth3) * 0.15
aRes4 = reson(aFilteredCarrier, kFormantFreq4, kBandwidth4) * 0.12
aRes5 = reson(aFilteredCarrier, kFormantFreq5, kBandwidth5) * 0.08

; Combine the formants to create the composite squawk sound
aSquawk = (aRes1 + aRes2 + aRes3 + aRes4 + aRes5) * 0.2

; Morph between FM sound and squawk based on kSquawk
aMorphSound = (1 - kSquawk) * aFilteredCarrier + kSquawk * aSquawk

; Apply a high-pass filter at around 2000 Hz to the combined sound
aFilteredSound butterhp aMorphSound, 10000
